...
class Sorcerer {
    ...
    draw() {
        ...
        if (this.fx > 0) {
            ...
            if (this.k === 'Ryu' || this.k === 'Yuta') {
                ...
                if(isClash) {
                    ...
                    ctx.beginPath(); 
                    ctx.arc(canvas.width/2, cy+5, 15 + Math.random()*20, 0, 7); 
                    ctx.fill();
                    for(let i=0; i<8; i++) {
        ...
        if (this.proj.active) {
            ...
            ctx.shadowBlur = 10; ctx.shadowColor = this.s.c;
            if (this.proj.type === 'NAIL') {
                ctx.fillRect(this.proj.x, this.proj.y + 15, 18 * this.dir, 4);
            } else {
                ctx.beginPath();
    ...
    spec(opp) {
        ...
        switch(this.k) {
            case 'Nobara': 
                this.proj = { active: true, x: this.x, y: this.y + 10, vx: this.dir * 24, type: 'NAIL' }; 
                this.spT = 450; break;
            case 'Hakari': 
                if (Math.random() < 0.4) { this.jackpot = 600; this.spT = 225; } 
                else { this.spT = 450; } 
                break;
    ...
    update(opp) {
        ...
        if (this.fx > 0) {
            ...
            if (isBeaming && !clashing && dist < 2000 && Math.abs(this.y - (opp.y+30)) < 100) {
...
window.addEventListener('touchstart', e => {
    e.preventDefault(); // CRITICAL: Stop double-tap zoom and scrolling
