...
class Sorcerer {
    ...
    spec(opp) {
        ...
        switch(this.k) {
            case 'Geto':
                for(let i=0; i<3; i++) { this.swarm.push({ x: this.x, y: this.y - 20 - (i*30), vx: this.dir * 18 }); }
                this.spT = 400; break;
    ...
    update(opp) {
        ...
        this.swarm = this.swarm.filter(p => {
            ...
            if (dist < 40) { opp.hp -= 15; opp.stun = 15; opp.vx = this.dir * 4; return false; }
    ...
    atk(opp) {
        ...
        if (Math.abs(this.x - opp.x) < 95 && Math.abs(this.y - opp.y) < 100) {
            if (this.inShadow) { opp.hp -= (this.s.d + 15); opp.stun = 70; } 
            else { opp.hp -= this.s.d; opp.stun = 12; }
            opp.vx = this.dir * 7;
        }
    }
